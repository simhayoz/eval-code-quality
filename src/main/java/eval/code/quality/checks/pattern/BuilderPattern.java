...
public class BuilderPattern extends DesignPattern {
    ...
    protected BooleanEvaluator getEvaluator(ContentProvider contentProvider) {
        ClassOrInterfaceDeclaration builder = contentProvider.findClassBy(builderName).orElseThrow(() -> new ClassNotFoundError(builderName));
        BooleanEvaluator evaluator = new BooleanEvaluator();
        Supplier<Boolean> hasConstructPart = () -> builder.getMethods().stream().anyMatch(m -> m.isPublic()
        ...
        Supplier<Boolean> hasBuildMethod = () -> builder.getMethods().stream().anyMatch(m -> m.isPublic()
                && m.getType().toString().equals(getSimpleName(productName)));
        evaluator.add(new BooleanSimple(hasBuildMethod, "builder has a method that build product"));
        evaluator.add(new BooleanSimple(() -> builder.getMethods().stream().filter(m -> m.isPublic()
                && m.getType().toString().equals(getSimpleName(productName)))
                .anyMatch(m -> m.getNameAsString().equals("build") || m.getNameAsString().equals("construct")),
                "method to build product is called 'build' or 'construct' for easier reading", false));
        return evaluator;
    }
