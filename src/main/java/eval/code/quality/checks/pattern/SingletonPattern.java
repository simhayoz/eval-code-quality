...
public class SingletonPattern extends DesignPattern {
    ...
    protected BooleanEvaluator getEvaluator(ContentProvider contentProvider) {
        ClassOrInterfaceDeclaration classCU = contentProvider.findClassBy(className).orElseThrow(() -> new ClassNotFoundError(className));
        BooleanEvaluator evaluator = new BooleanEvaluator();
        List<ConstructorDeclaration> constructor = classCU.getConstructors();
        List<MethodDeclaration> methods = classCU.getMethods().stream().filter(m -> m.isStatic() && m.isPublic() && m.getType().toString().equals(getSimpleName(className))).collect(Collectors.toList());
        List<FieldDeclaration> variables = classCU.getFields().stream().filter(v -> v.isStatic() && v.getVariables().stream().anyMatch(e -> e.getType().toString().equals(getSimpleName(className)))).collect(Collectors.toList());
        evaluator.add(() -> !classCU.isInterface(), "class is not an interface");
        evaluator.add(() -> constructor.size() == 1 && constructor.get(0).isPrivate(), "constructor is unique and private");
        evaluator.add(() -> variables.size() == 1, "there exists a unique static variable of type: " + getSimpleName(className));
        Supplier<Boolean> staticInit = () -> variables.get(0).isFinal()
        ...
        Supplier<Boolean>  publicInstanceVariable = () -> variables.get(0).isPublic()
                && staticInit.get();
